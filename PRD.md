...
* **Vendor Canonicalization (Post-Ingestion):** Ingested statements from different sources do not guarantee a stable spelling for the same vendor, so "TMOBILE" and "T-Mobile" (or "Quickpark" and "Quick Park") currently land as distinct vendors and fragment spending. After ingestion, a scan operation must detect such typographically mismatched but semantically duplicate vendors and consolidate them into a single canonical name, so that their amounts are coherently aggregated: because each row keeps its canonical signed amount (negative for expenditures, positive for deposits), consolidated spending per vendor is obtained by summing — or, for deposits, netting/subtracting — across the merged rows. Consolidation is performed **by renaming**, never by deleting rows or mutating per-row amounts, so the ledger stays lossless, auditable, and the operation is idempotent. This ensures analytics views (vendor lists, category drill-downs) reflect consolidated spending per vendor rather than fragmented entries.
